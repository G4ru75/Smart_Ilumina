...
class ConfigLuzModal extends StatelessWidget {
  final String luzId;
  final VoidCallback? onSaved;

  const ConfigLuzModal({super.key, required this.luzId, this.onSaved});

  @override
  Widget build(BuildContext context) {
    final lucesController = Get.find<LucesController>();

    // **StreamBuilder directo - No necesita habitacionId**
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || !snapshot.data!.exists) {
          return const Center(child: Text('Luz no encontrada'));
        }

        final luz = Luces.fromMap(
          snapshot.data!.data() as Map<String, dynamic>,
        );
        final titulo = 'Configurar: ${luz.nombre}';
        final colorActual = luz.color;
              ...
              Row(
                children: [
                  ...
                  Switch(
                    ...
                    onChanged: (v) =>
                        lucesController.cambiarEstadoLuz(luz.id, v),
                  ),
                ],
              ),
